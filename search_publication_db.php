 ...
 */
function quickSearch(&$pub_id_array) {
    global $option;
 ...
 */
function advncedSearch(&$pub_id_array) {
    global $option;
...
function cvFormCreate($pub_id_array) {
    ...
    $form = new HTML_QuickForm('cvForm', 'post', 'cv.php', '_self',
                               'multipart/form-data');
...
// QUICK SEARCH ---------------------------------------------------------------
if($option->search != "") {
    $pub_id_array = quickSearch($pub_id_array);
}
else {
	//ADVANCED SEARCH
    $pub_id_array = advncedSearch($pub_id_array);
}
